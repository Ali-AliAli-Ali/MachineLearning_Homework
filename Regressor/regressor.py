...
if mode == 'lin':
    ...
    for i in range(0, n):
        x = i
        maxX = max(maxX, x)
        y = i*random.randint(20, 50) #random.randint(0, i*20)
        plt.scatter(x, y)
        A[i][1] = AT[1][i] = x
        Y[i][0] = y
    ...
    for i in range(0, n):
        x = i
        maxX = max(maxX, x)
        y = 1000*np.arctan(i-500)-i + random.randint(-100, 200)
        plt.scatter(x, y)
    ...
    for i in range(0, n):
        x = i
        maxX = max(maxX, x)
        y = -i*i + i*40 + 405 #random.randint(1, (i+1)*20)#exp(i)
        plt.scatter(x, y)
        A[i][1] = AT[1][i] = x
        Y[i][0] = log(y)
